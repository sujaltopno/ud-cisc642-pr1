...
M = csvread('given_computer_perspective.csv',1,0); %reading the CSV file, changing names to read different csv files from the same folder
...
tformB = fitgeotrans(fixedPoints,movingPoints,'projective'); %creating tformB object from built-in function
outputImage = imwarp(I, tformB); %applying transform to image
O = tformB.T; %creating the X2 matrix from the tformB object

disp('Matrix using built-in function');
disp(O'); %displaying the matrix

%% Generating X2 using Self coded function
for i = 1:10
...
tform = projective2d(H); %creating tform object from matrix H
